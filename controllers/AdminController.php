...
class AdminController {
    ...
    /**
     * CRUD khu cũ theo module buildings. Giữ lại để tương thích route cũ.
     */
    ...
    public function areas() {
        ...
        require_once BASE_PATH . 'views/admin/areas.php';
    }
    ...
    public function floors() {
        ...
        require_once BASE_PATH . 'views/admin/floors.php';
    }
    ...
    public function amenities() {
        ...
        require_once BASE_PATH . 'views/admin/amenities.php';
    }
    ...
    public function contracts() {
        ...
        require_once BASE_PATH . 'views/admin/contracts.php';
    }
    ...
    public function invoices() {
        ...
        require_once BASE_PATH . 'views/admin/invoices.php';
    }
    ...
    public function services() {
        ...
        require_once BASE_PATH . 'views/admin/services.php';
    }

    /**
     * Trang đổi giá dịch vụ theo tháng hiệu lực tương lai.
     */
    public function priceChanges() {
        ...
        require_once BASE_PATH . 'views/admin/price_changes.php';
    }
    ...
    public function savePriceChange() {
        ...
        try {
            ...
            setFlash('admin_price_change_message', 'Đã lưu lịch sử đổi giá và gửi thông báo cho tenant.');
        } catch (Throwable $exception) {
    ...
    /**
     * Trang gửi thông báo thủ công và xem lịch sử thông báo đã phát hành.
     */
    ...
    public function comments() {
        ...
        require_once BASE_PATH . 'views/admin/comments.php';
    }
    ...
    public function bannedWords() {
        ...
        require_once BASE_PATH . 'views/admin/banned_words.php';
    }
    ...
    public function commentReports() {
        ...
        require_once BASE_PATH . 'views/admin/comment_reports.php';
    }
    ...
    public function saveService() {
        ...
        if ($id > 0 && !ServiceModel::getById($id)) {
            setFlash('admin_service_error', 'Dịch vụ cần cập nhật không tồn tại.');
            redirectTo('admin-services', $returnRoomId > 0 ? ['room_id' => $returnRoomId] : []);
        }
        ...
        if (!in_array($data['billing_mode'], $this->getAllowedServiceBillingModes(), true)) {
            setFlash('admin_service_error', 'Cách tính giá không hợp lệ.');
            setFlash('admin_service_old', array_merge($data, ['id' => $id]));
            redirectTo('admin-services', $redirectParams);
        }
        ...
        $savedId = ServiceModel::save($data, $id > 0 ? $id : null);
        setFlash('admin_service_message', $id > 0 ? 'Đã cập nhật dịch vụ thành công.' : 'Đã thêm dịch vụ mới thành công.');
        redirectTo('admin-services', array_filter([
            'edit' => $savedId,
            'room_id' => $returnRoomId > 0 ? $returnRoomId : null,
        ], static fn($value) => $value !== null && $value !== ''));
    }
    ...
    /**
     * Xóa dịch vụ khi được phép. Dịch vụ bắt buộc sẽ bị chặn ở tầng model.
     */
    ...
    public function meterReadings() {
        ...
        require_once BASE_PATH . 'views/admin/meter_readings.php';
    }
    ...
    /**
     * Lưu khu cũ của module buildings.
     */
    ...
    /**
     * Xóa khu cũ của module buildings.
     */
    ...
    public function saveArea() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($data['name'] !== '') {
                AreaModel::save($data, $id > 0 ? $id : null);
            }
        }
    ...
    public function deleteFloor($id) {
        ...
        if ($floor) {
            ...
            redirectTo('admin-floors', ['area_id' => (int)($floor['area_id'] ?? 0)]);
        }
    ...
    /**
     * Lưu phòng hoặc cập nhật nhanh trạng thái phòng từ danh sách.
     */
    public function saveRoom() {
        ...
        $id = (int)($_POST['id'] ?? 0);
        $status = $this->normalizeRoomStatus($_POST['status'] ?? 'available', 'available');
        ...
        $data = [
            ...
            'max_occupancy' => (int)($_POST['max_occupancy'] ?? 0),
            'description' => trim((string)($_POST['description'] ?? '')),
            'thumbnail' => trim((string)($_POST['thumbnail'] ?? '')),
            'status' => $status,
        ];

        $formState = array_merge($data, [
            'id' => $id,
            'area_id' => (int)($_POST['area_id'] ?? 0),
        ]);
        ...
        if (!$floor) {
            ...
            redirectTo('admin-rooms', array_filter([
        ...
        if ($data['name'] === '') {
            setFlash('admin_room_error', 'Tên phòng là bắt buộc.');
            setFlash('admin_room_old', $formState);
            redirectTo('admin-rooms', array_filter([
        ...
        if ($data['price'] <= 0) {
            setFlash('admin_room_error', 'Giá phòng phải lớn hơn 0.');
            setFlash('admin_room_old', $formState);
            redirectTo('admin-rooms', array_filter([
                'area_id' => (int)($floor['area_id'] ?? 0),
                'edit' => $id > 0 ? $id : null,
        ...
        if ($id > 0 && !RoomModel::getById($id)) {
            setFlash('admin_room_error', 'Phòng cần cập nhật không tồn tại.');
            redirectTo('admin-rooms', $redirectParams);
        }

        RoomModel::save($data, $id > 0 ? $id : null);
        setFlash('admin_room_message', $id > 0 ? 'Đã cập nhật phòng thành công.' : 'Đã thêm phòng mới thành công.');
        redirectTo('admin-rooms', [
            'area_id' => (int)($floor['area_id'] ?? 0),
    ...
    public function deleteRoom($id) {
        ...
        if (RoomModel::hasActiveOccupants($id)) {
            setFlash('admin_room_error', 'Phòng đang có người ở!');
            redirectTo('admin-rooms', $redirectParams);
        }
