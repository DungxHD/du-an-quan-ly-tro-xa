...
$c = preg_replace('/private\s+const\s+APPLIES_TO\s*=\s*\[\s*\'room\'\s*,\s*\'person\'\s*\]\s*;/', <<<'EOT'
...
$c = preg_replace('/\'is_active\'\s*=>\s*array_key_exists\(\s*\'is_active\'\s*,\s*\$service\s*\)\s*\?\s*\(\s*!empty\(\s*\$service\[\s*\'is_active\'\s*\]\s*\)\s*\?\s*1\s*:\s*0\s*\)\s*:\s*1\s*,/', <<<'EOT'
...
$c = preg_replace('/if\s*\(\s*\(int\)\(\s*\$service\[\s*\'is_required\'\s*\]\s*\?\?\s*0\s*\)\s*===\s*1\s*\)\s*\{[\s\S]*?throw\s+new\s+RuntimeException\(\s*\'Dịch vụ bắt buộc không thể xóa\.\'\s*\)\s*;\s*\}/', <<<'EOT'
...
$c = preg_replace('/public\s+static\s+function\s+save\(\s*array\s+\$data\s*,\s*\$id\s*=\s*null\s*\)\s*\{[\s\S]*?\$payload\s*=\s*self::normalizeServiceRow\(\s*\$data\s*\)\s*;/', <<<'EOT'
...
$c = preg_replace('/\'service_icon\'\s*=>\s*trim\(\s*\(string\)\(\s*\$row\[\s*\'service_icon\'\s*\]\s*\?\?\s*\'settings\'\s*\)\s*\)\s*,/', <<<'EOT'
...
$c = preg_replace('/public\s+static\s+function\s+saveChange\([\s\S]*?public\s+static\s+function\s+getEffectivePriceForPeriod/', <<<'EOT'
...
$c = preg_replace('/public\s+static\s+function\s+getEffectivePriceForPeriod\([\s\S]*?public\s+static\s+function\s+buildNotificationContent/', <<<'EOT'
...
EOT;
$c = preg_replace('/public\s+function\s+services\(\)\s*\{[\s\S]*?public\s+function\s+priceChanges\(\)\s*\{/', $newServices . "\n    public function priceChanges()\n    {\n        PriceChangeModel::applyDueChanges();", $c, 1, $cnt);
...
EOT;
$c = preg_replace('/public\s+function\s+savePriceChange\(\)\s*\{[\s\S]*?public\s+function\s+notifications\(\)\s*\{/', $newSavePrice . "\n    public function notifications()\n    {", $c, 1, $cnt);
...
EOT;
$c = preg_replace('/public\s+function\s+saveService\(\)\s*\{[\s\S]*?public\s+function\s+deleteService\(\s*\$id\s*\)\s*\{/', $newSaveService . "\n    public function deleteService(\$id)\n    {", $c, 1, $cnt);
...
$c = preg_replace('/PriceChangeModel::getEffectivePriceForPeriod\(\s*\$serviceId\s*,\s*\(int\)\$month\s*,\s*\(int\)\$year\s*,\s*\(float\)\(\s*\$service\[\s*\'price\'\s*\]\s*\?\?\s*0\s*\)\s*\)\s*;\s*\$billingMode\s*=\s*\$service\[\s*\'billing_mode\'\s*\]\s*\?\?\s*\'fixed\'\s*;/', <<<'EOT'
...
$c = preg_replace('/private\s+static\s+function\s+resolveInitialIndexField\(\s*array\s+\$service\s*\)\s*\{[\s\S]*?private\s+static\s+function\s+getPreviousPeriod\(\s*/', <<<'EOT'
