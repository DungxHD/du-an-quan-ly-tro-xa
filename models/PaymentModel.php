...
class PaymentModel {
    ...
    /**
     * Chốt giá tiền phòng theo rule an toàn:
     * ưu tiên giá phòng chung, chỉ rơi về `contracts.rent_price` khi không có giá phòng hợp lệ.
     */
    private static function resolveRoomRentPrice(array $room, $billingContract, array $contracts) {
        ...
        if (!empty($billingContract) && (float)($billingContract['rent_price'] ?? 0) > 0) {
            return (float)$billingContract['rent_price'];
        }

        foreach ($contracts as $contract) {
            if ((float)($contract['rent_price'] ?? 0) > 0) {
                return (float)$contract['rent_price'];
            }
        }
